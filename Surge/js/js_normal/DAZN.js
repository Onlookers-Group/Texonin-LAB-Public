const DEFAULT_OPTIONS = {
    title: 'Ready to check DAZN.',
    availableContent: 'You can watch DAZN in #REGION_CODE#.',
    availableIcon: 'checkmark.seal',
    availableIconColor: '#16A951',
    availableStyle: 'good',
    comingtitle: 'Ready to check DAZN.',
    comingContent: 'Soon you can watch DAZN in #REGION_CODE#!',
    comingIcon: 'exclamationmark.triangle',
    ...
    errortitle: 'Ready to check DAZN.',
...
;(async () => {
    let { isAllowed, region } = await Promise.race([getLocationInfo(), timeout(options.timeout)])
    switch (isAllowed) {
        case true:
...
function getLocationInfo() {
  return new Promise((resolve, reject) => {
    let opts = {
      ...
      body: JSON.stringify({"LandingPageKey":"generic","Languages":"zh-CN,zh,zh-TW,en","Platform":"web","PlatformAttributes":{},"Manufacturer":"","PromoCode":"","Version":"2"}),
    }
    $httpClient.post(opts, function (error, response, data) {
      ...
      let isAllowed = data.Region ? data.Region.isAllowed : false
      let region = data.Region ? data.Region.Country : false
      resolve({ isAllowed, region })
    })
  })
}
